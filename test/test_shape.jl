@testset "Sphere" begin
    @testset "from_above" begin
        ...
        shape = Sphere()
        intersect = ray_intersection(shape, ray) 
        @test intersect ≈ HitRecord(Point(0, 0, 1), VEC_Z, Vec2D(NaN,0), 1, ray)
    end
    ...
    @testset "from_behind" begin
        ...
        shape = Sphere()
        intersect = ray_intersection(shape, ray) 
        @test intersect ≈ HitRecord(Point(1, 0, 0), VEC_X, Vec2D(0, 0.5), 2, ray)
    end
    ...
    @testset "from_within" begin
        ...
        shape = Sphere()
        intersect = ray_intersection(shape, ray) 
        @test intersect ≈ HitRecord(Point(1, 0, 0), -VEC_X, Vec2D(0, 0.5), 1, ray)
    end
    ...
    @testset "transposed_from_above" begin
        ...
        intersect = ray_intersection(shape, ray) 
        @test intersect ≈ HitRecord(Point(10, 0, 1), VEC_Z, Vec2D(NaN, 0), 1, ray)
    end
    ...
    @testset "transposed_from_behind" begin
        ...
        shape = Sphere(translation(Vec(10,0,0)))
        intersect = ray_intersection(shape, ray) 
        @test intersect ≈ HitRecord(Point(11, 0, 0), VEC_X, Vec2D(0, 0.5), 2, ray)
    end
