...
@testset "Sphere" begin
    @testset "from_above" begin
        ...
        @test intersect ≈ HitRecord(Point{Float64}(0, 0, 1), normal_z(Float64), Vec2D{Float64}(0, 0), 1., ray, Material())
    end
    ...
    @testset "from_behind" begin
        ...
        shape = Sphere()
        intersect = ray_intersection(ray, shape) 
        @test intersect ≈ HitRecord(Point{Float64}(1, 0, 0), normal_x(Float64), Vec2D{Float64}(0.5, 0.5), 2., ray, Material())
    end
    ...
    @testset "from_within" begin
        ...
        shape = Sphere()
        intersect = ray_intersection(ray, shape) 
        @test intersect ≈ HitRecord(Point{Float64}(1, 0, 0), -normal_x(Float64), Vec2D{Float64}(0.5, 0.5), 1., ray, Material())
    end
    ...
    @testset "transposed_from_above" begin
        ...
        intersect = ray_intersection(ray, shape) 
        @test intersect ≈ HitRecord(Point{Float64}(10, 0, 1), normal_z(Float64), Vec2D{Float64}(0, 0), 1., ray, Material())
    end
    ...
    @testset "transposed_from_behind" begin
        ...
        shape = Sphere(transformation = translation(Vec(10,0,0)))
        intersect = ray_intersection(ray, shape) 
        @test intersect ≈ HitRecord(Point{Float64}(11, 0, 0), normal_x(Float64), Vec2D{Float64}(0.5, 0.5), 2., ray, Material())
    end
...
@testset "Plane" begin
    @testset "from_above" begin
        ...
        shape = Plane()
        intersect = ray_intersection(ray, shape) 
        @test intersect ≈ HitRecord(Point{Float64}(0, 0, 0), normal_z(Float64), Vec2D{Float64}(0,0), 1., ray, Material())
    end
    ...
    @testset "transposed_from_above" begin
        ...
        intersect = ray_intersection(ray, shape) 
        @test intersect ≈ HitRecord(Point{Float64}(1.5, 0.5, 1), normal_z(Float64), Vec2D{Float64}(0.5, 0.5), 2., ray, Material())
    end
    ...
    @testset "from_diagonal" begin
        ...
        shape = Plane()
        intersect = ray_intersection(ray, shape) 
        @test intersect ≈ HitRecord(Point{Float64}(1., 1., 0.), normal_z(Float64), Vec2D{Float64}(0., 0.), 1., ray, Material())
    end
end