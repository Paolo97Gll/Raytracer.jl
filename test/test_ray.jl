...
@testset "constructor" begin
    ray1 = Ray{Float64}(Point(1.0, 2.0, 3.0), Vec(5.0, 4.0, -1.0))
    ray2 = Ray{Float64}(Point(1.0, 2.0, 3.0), Vec(5.0, 4.0, -1.0))
    ray3 = Ray{Float64}(Point(5.0, 1.0, 4.0), Vec(3.0, 9.0, 4.0))
...
@testset "methods" begin
    ray = Ray{Float64}(Point(1.0, 2.0, 4.0), Vec(4.0, 2.0, 1.0))
    @test ray(0.0) ≈ ray.origin
    @test ray(1.0) ≈ Point(5.0, 4.0, 5.0)
    @test ray(2.0) ≈ Point(9.0, 6.0, 6.0)

    ray = Ray{Float64}(Point(1.0, 2.0, 3.0), Vec(1.0, 2.0, 3.0))
