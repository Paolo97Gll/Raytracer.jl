...
@testset "Cylinder" begin
    @testset "from_above" begin
        ...
        shape = Cylinder()
        intersect = ray_intersection(ray, shape)
        @test intersect ≈ HitRecord(Point(0f0, 0f0, 5f-1), NORMAL_Z_false, Vec2D(3//4, 3//4), 1.5f0, ray, Material())
    end
    ...
    @testset "from_behind" begin
        ...
        shape = Cylinder()
        intersect = ray_intersection(ray, shape)
        @test intersect ≈ HitRecord(Point(5f-1, 0f0, 0f0), NORMAL_X_false, Vec2D(1//2, 1//2), 2.5f0, ray, Material())
    end
    ...
    @testset "from_within" begin
        ...
        shape = Cylinder()
        intersect = ray_intersection(ray, shape)
        @test intersect ≈ HitRecord(Point(5f-1, 0f0, 0f0), -NORMAL_X_false, Vec2D(1//2, 1//2), 5f-1, ray, Material())
    end

    @testset "transposed_from_above" begin
        ray = Ray(Point(10f0, 0f0, 2f0), -VEC_Z)
        shape = Cylinder(transformation = translation(Vec(10f0, 0f0, 0f0)))
        intersect = ray_intersection(ray, shape)
        @test intersect ≈ HitRecord(Point(10f0, 0f0, 5f-1), NORMAL_Z_false, Vec2D(3//4, 3//4), 1.5f0, ray, Material())
    end
    ...
    @testset "transposed_from_behind" begin
        ...
        shape = Cylinder(transformation = translation(Vec(10f0, 0f0, 0f0)))
        intersect = ray_intersection(ray, shape)
        @test intersect ≈ HitRecord(Point(10.5f0, 0f0, 0f0), NORMAL_X_false, Vec2D(1//2, 1//2), 2.5f0, ray, Material())
    end
