...
function Base.showerror(io::IO, e::InterpreterError)
    print(io, typeof(e))
    printstyled(" @ ", e.location, color=:light_black)
    println()
    printstyled(e.msg, color=:red) 
    println()
    printstyled("source: ", color=:light_black)
    println(read_at_line(e.location.file_name, e.location.line_num))
    printstyled(" " ^ (e.location.col_num + 6), color=:light_black)
    printstyled("^" ^ e.len, color=:red)
end