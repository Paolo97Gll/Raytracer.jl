...
Members:
- `aspect_ratio` is a `Float32` and defines how larger than the height is the image (16/9, 4/3, ...). You can use julia rational like `16//9`.
- `transformation` is an instance of [`Transformation`](@ref) and define the transformation applied to the rays generated by the camera.
"""
...
- `aspect_ratio` is a `Float32` and defines how larger than the height is the image (16/9, 4/3, ...). You can use julia rational like `16//9`.
- `transformation` is an instance of [`Transformation`](@ref) and define the transformation applied to the rays generated by the camera.
- `screen_distance` tells how much far from the eye of the observer is the screen and it influences the FOV (field-of-view).
"""
